// Saskia Perret-Gentil (11-210-259)
// Martin Arnold (09-716-556)
/* ************************************************************************* *\
...
public class GuessOrc {
	...
	public int calculateHint( int guessedMineShaftId ) {
		if (Math.abs(mineShaftId-guessedMineShaftId) > 5)
			return 4;
		else if (mineShaftId < guessedMineShaftId)
			return 0;
		else if (mineShaftId > guessedMineShaftId)
			return 1;
		// there is other cases (under dirt or under stone)
		// but I don't know how to implement them
		// also would be nice to give randomly one hint when several could apply
		return 0;
	}
	...
	public void start() {
		...
		// init chieftain position randomly
		...
		while(nbOfAttempts < MAX_ATTEMPTS) {
			System.out.print("Which mine shaft would you like to flush (a-l)? ");
			Scanner scan = new Scanner(System.in);
			c = scan.next().charAt(0);
			column = getColumnAsInt(c);
			hintId = calculateHint(column);
			//System.out.println("chieftain: " + getMineShaftAsChar(mineShaftId));
			//System.out.println("hint: " + HINTS[hintId]);

			printMap(column, hintId);
			nbOfAttempts += 1;
			// don't know what to do to get the victory condition
			// maybe access the hasWon variable in printMap() function?
		}
	}
